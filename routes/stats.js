...
const { execRedisCommand, execRedisPipelinedCommands, parseRedisInfo } = require('../services/redis');
        ...
        for (const [key, value] of Object.entries(keyspace)) {
            if (key.startsWith('db')) {
                ...
                if (expiresMatch && keysMatch) {
                    ...
                    keysWithoutTTL += parseInt(keysMatch[1]) - parseInt(expiresMatch[1]);
                }
            }
        }
...
router.get('/commandstats', async (req, res) => {
    try {
        ...
        const commandStatsResult = await execRedisCommand('INFO COMMANDSTATS', env);
            ...
            if (key.startsWith('cmdstat_')) {
                ...
                if (callsMatch) {
                    ...
                    commands.push({
                        ...
                        usec_per_call: parseFloat(usecPerCallMatch ? usecPerCallMatch[1] : 0),
...
router.get('/memory-by-pattern', async (req, res) => {
    try {
        ...
        while (hasMore && scannedKeys < sampleSize) {
            const scanResult = await require('../services/redis').scanClusterNodes(
                '*', 
                cursors, 
        ...
        if (!fastMode && collectedKeys.length > 0) {
            const commands = collectedKeys.map(({ key }) => `MEMORY USAGE "${key}"`);
            console.log(`Pipelining ${commands.length} MEMORY USAGE commands for all sampled keys`);
            
            const results = await execRedisPipelinedCommands(commands, env);
            
            console.log(`Received ${results.length} results, sample (first 5): ${results.slice(0, 5).join(', ')}`);
            
            for (let i = 0; i < collectedKeys.length && i < results.length; i++) {
            ...
            console.log(`Memory calculated for ${Object.keys(patterns).length} patterns`);
        }
        ...
        const patternArray = Object.values(patterns).map(p => ({
            ...
            avg_memory: p.count > 0 && p.total_memory > 0 ? Math.round(p.total_memory / p.count) : 0,
        ...
        if (fastMode) {
            ...
            const totalMemory = patternArray.reduce((sum, p) => sum + p.total_memory, 0);
            
            patternArray.forEach(p => {
                p.percentage = totalMemory > 0 ? ((p.total_memory / totalMemory) * 100).toFixed(2) : 0;
            });
        ...
        res.json({
            ...
            total_memory: patternArray.reduce((sum, p) => sum + p.total_memory, 0),
            fast_mode: fastMode
        });
            ...
            if (parts.length >= 2) {
                ...
                for (let i = 0; i < parts.length; i++) {
                    ...
                    if (/^\d+$/.test(part)) {
                        ...
                        patternParts.push('*');
                        break;
                    } else if (/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(part)) {
                        patternParts.push('*');
                        break;
                    } else {
...
router.get('/slowlog', async (req, res) => {
    try {
        ...
        const slowlogResult = await execRedisCommand(`SLOWLOG GET ${count}`, env);
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (/^\d+\)$/.test(line)) {
                ...
                while (j < lines.length && !/^\d+\)$/.test(lines[j])) {
                    ...
                    if (part && !part.startsWith(')') && part !== '(empty array)') {
