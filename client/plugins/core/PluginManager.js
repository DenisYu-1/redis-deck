export class PluginManager {
    ...
    async loadConfig() {
        try {
            ...
            if (!baseResponse.ok) {
                throw new Error(
                    `Failed to load base config: ${baseResponse.statusText}`
                );
            }
            ...
            try {
                ...
                console.error(
                    'No override config found, using base config only'
                );
            }
    ...
    async loadPlugin(pluginConfig) {
        try {
            ...
            if (!PluginClass) {
                throw new Error(
                    `Plugin ${pluginConfig.id} must export a default class`
                );
            }
    ...
    async initializePlugins(context) {
        const sorted = Array.from(this.plugins.values()).sort(
