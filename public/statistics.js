...
async function loadRealtimeStats() {
    try {
        const response = await fetch(`/api/stats/realtime?env=${encodeURIComponent(currentEnvironment)}`);
        const data = await response.json();
        
        document.getElementById('metric-memory').textContent = data.memory.used_human || '-';
        ...
        document.getElementById('metric-memory-subtitle').textContent = subtitle;
        ...
        document.getElementById('metric-keys').textContent = totalKeys.toLocaleString();
        ...
        document.getElementById('metric-clients').textContent = data.clients.connected || 0;
        document.getElementById('metric-evictions').textContent = 
            (data.stats.evicted_keys || 0).toLocaleString();
        
        const fragRatio = data.memory.fragmentation || 0;
        document.getElementById('metric-fragmentation').textContent = fragRatio.toFixed(2);
        document.getElementById('metric-fragmentation-subtitle').textContent = 
            fragRatio > 1.5 ? '⚠️ High fragmentation' : 
            fragRatio < 1 ? '⚠️ Low ratio (swapping?)' : '✓ Healthy';
        
        document.getElementById('metric-ops').textContent = (data.stats.ops_per_sec || 0).toLocaleString();
        document.getElementById('metric-ops-subtitle').textContent = 
            data.stats.ops_per_sec > 1000 ? 'High load' : data.stats.ops_per_sec > 100 ? 'Moderate' : 'Low activity';
        
        const cpuTotal = (data.cpu.used_cpu_sys || 0) + (data.cpu.used_cpu_user || 0);
        document.getElementById('metric-cpu').textContent = cpuTotal.toFixed(2) + 's';
        ...
        const totalNet = (data.stats.total_net_input_bytes || 0) + (data.stats.total_net_output_bytes || 0);
        const netGB = totalNet / 1024 / 1024 / 1024;
        document.getElementById('metric-network').textContent = netGB > 1 ? netGB.toFixed(2) + ' GB' : (totalNet / 1024 / 1024).toFixed(2) + ' MB';
        ...
        document.getElementById('metric-total-conns').textContent = totalConns.toLocaleString();
        document.getElementById('metric-total-conns-subtitle').textContent = 
            rejectedConns > 0 ? `⚠️ ${rejectedConns} rejected` : '✓ None rejected';
        ...
        const ttlPercentage = totalKeysForTTL > 0 ? ((keysWithTTL / totalKeysForTTL) * 100).toFixed(1) : 0;
        document.getElementById('metric-ttl').textContent = keysWithTTL.toLocaleString();
...
async function loadHistoricalData() {
    try {
        ...
        if (!result.data || result.data.length === 0) {
            showToast('No historical data available yet. Data will be collected over time.', 'warning');
            document.getElementById('memory-chart-loading').textContent = 'No data available yet';
            document.getElementById('keys-chart-loading').textContent = 'No data available yet';
            document.getElementById('hitrate-chart-loading').textContent = 'No data available yet';
            document.getElementById('ops-chart-loading').textContent = 'No data available yet';
            document.getElementById('network-chart-loading').textContent = 'No data available yet';
            return;
        }
        ...
        document.getElementById('hitrate-chart-loading').classList.add('hidden');
        document.getElementById('ops-chart-loading').classList.add('hidden');
        document.getElementById('network-chart-loading').classList.add('hidden');
...
function renderMemoryChart(data) {
    ...
    const memoryDataMB = data.map(d => (d.used_memory / 1024 / 1024).toFixed(2));
    const peakDataMB = data.map(d => (d.used_memory_peak / 1024 / 1024).toFixed(2));
...
function renderNetworkChart(data) {
    ...
    const inputDataMB = data.map(d => ((d.total_net_input_bytes || 0) / 1024 / 1024).toFixed(2));
    const outputDataMB = data.map(d => ((d.total_net_output_bytes || 0) / 1024 / 1024).toFixed(2));
...
async function loadCommandStats() {
    try {
        const response = await fetch(`/api/stats/commandstats?env=${encodeURIComponent(currentEnvironment)}`);
        const data = await response.json();
        ...
        if (!data.commands || data.commands.length === 0) {
            table.innerHTML = '<tr><td colspan="5" style="padding: 20px; text-align: center; color: var(--text-secondary);">No command statistics available</td></tr>';
        ...
        table.innerHTML = data.commands.map(cmd => {
            ...
            const rateColor = cmd.calls_per_sec > 100 ? 'var(--accent-danger)' : 
                             cmd.calls_per_sec > 10 ? 'var(--accent-warning)' : 'var(--text-primary)';
        ...
        document.getElementById('command-stats-timeframe').textContent = 'Error loading';
    }
}

async function loadSlowlog() {
    try {
        const response = await fetch(`/api/stats/slowlog?env=${encodeURIComponent(currentEnvironment)}&count=10`);
        const data = await response.json();
        
        const table = document.getElementById('slowlog-table');
        
        if (!data.slowlog || data.slowlog.length === 0) {
            table.innerHTML = '<tr><td colspan="4" style="padding: 20px; text-align: center; color: var(--text-secondary);">No slow queries logged</td></tr>';
            return;
        }
        
        table.innerHTML = data.slowlog.map(entry => {
            const date = new Date(entry.timestamp * 1000);
            const timeStr = date.toLocaleString();
            const durationMs = (entry.duration_us / 1000).toFixed(2);
            const durationColor = entry.duration_us > 100000 ? 'var(--accent-danger)' : entry.duration_us > 10000 ? 'var(--accent-warning)' : 'var(--text-primary)';
...
async function analyzeMemoryPatterns(fastMode = true) {
    try {
        ...
        statusEl.textContent = fastMode ? 'Scanning keys...' : 'Scanning keys and calculating memory...';
        ...
        document.getElementById('pattern-analysis-loading').style.display = 'none';
...
function renderPatternPieChart(patterns, fastMode = false) {
    ...
    const data = fastMode ? top10.map(p => p.count) : top10.map(p => p.total_memory);
...
function renderPatternSummary(patterns, totalMemory, fastMode = false) {
    ...
    summaryEl.innerHTML = top5.map((pattern, index) => {
...
function renderPatternTable(patterns, fastMode = false) {
    ...
    tableEl.innerHTML = patterns.map(pattern => {
        const totalMB = (pattern.total_memory / 1024 / 1024).toFixed(2);
        const avgKB = (pattern.avg_memory / 1024).toFixed(2);
        const sampleKeys = pattern.sample_keys.map(k => `<code style="background: var(--bg-code); padding: 2px 4px; border-radius: 3px; font-size: 0.85em;">${k}</code>`).join(' ');
        
        const memoryDisplay = fastMode ? 'N/A (fast mode)' : `${totalMB} MB`;
...
function resetAllData() {
    ...
    document.getElementById('metric-memory-subtitle').textContent = 'Loading...';
    document.getElementById('memory-progress').style.width = '0%';
    
    document.getElementById('metric-keys').textContent = '-';
    document.getElementById('metric-keys-subtitle').textContent = 'Across all databases';
    
    document.getElementById('metric-hitrate').textContent = '-';
    document.getElementById('metric-hitrate-subtitle').textContent = 'Cache effectiveness';
    
    document.getElementById('metric-clients').textContent = '-';
    document.getElementById('metric-clients-subtitle').textContent = 'Active connections';
    
    document.getElementById('metric-evictions').textContent = '-';
    document.getElementById('metric-evictions-subtitle').textContent = 'Due to memory limit';
    
    document.getElementById('metric-fragmentation').textContent = '-';
    document.getElementById('metric-fragmentation-subtitle').textContent = 'RSS/Used ratio';
    
    document.getElementById('metric-ops').textContent = '-';
    document.getElementById('metric-ops-subtitle').textContent = 'Current throughput';
    
    document.getElementById('metric-cpu').textContent = '-';
    document.getElementById('metric-cpu-subtitle').textContent = 'System + User';
    
    document.getElementById('metric-network').textContent = '-';
    document.getElementById('metric-network-subtitle').textContent = 'Total transferred';
    
    document.getElementById('metric-total-conns').textContent = '-';
    document.getElementById('metric-total-conns-subtitle').textContent = 'Lifetime received';
    ...
    document.getElementById('command-stats-table').innerHTML = '<tr><td colspan="5" style="padding: 20px; text-align: center; color: var(--text-secondary);">Loading...</td></tr>';
    document.getElementById('command-stats-timeframe').textContent = 'Loading...';
    
    document.getElementById('slowlog-table').innerHTML = '<tr><td colspan="4" style="padding: 20px; text-align: center; color: var(--text-secondary);">Loading...</td></tr>';
